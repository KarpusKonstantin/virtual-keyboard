...
class VirtualKeyboard {
  ...
  createButtonRow(keyboard, buttonArrayRow) {
    ...
    for (const button of buttonArrayRow) {
      ...
      const spanRusVar = `<span class="noShift hidden">${button.rus_eng.noShift[0]}</span>
      ...
      const spanEngVar = `<span class="noShift">${button.rus_eng.noShift[1]}</span>
      ...
      spanRus.innerHTML = spanRusVar;
      spanEng.innerHTML = spanEngVar;
  ...
  printText(event) {
    const keyChildSpan = document.querySelectorAll(`.${event.target.dataset.button} span span `);

    if (keyChildSpan.length > 0) {
      /* Target DIV и значет мы ищем чего написать */
      keyChildSpan.forEach((item) => {
  ...
  mouseUpEvent() {
    const q = document.querySelectorAll('.active');
  ...
  keyDownEvent(event) {
    ...
    const button = document.querySelector(`.${codeKey}`);
  ...
  changeWordCase() {
    const q = document.querySelectorAll(`.${this.lang} span`);
  ...
  changeLanguage() {
    const q = document.querySelectorAll('span.rus, span.eng');
